...
class OracleNetwork(ResnetModel):
    ...
    def __init__(self, config, num_words, device='', reuse=False, scopename="oracle"):
        ResnetModel.__init__(self, scopename, device=device)
        ...
        with tf.variable_scope(self.scope_name, reuse=reuse) as scope:
            ...
            # History
            self._history = tf.placeholder(tf.int32, [self.batch_size, None], name='history')
            self._hist_seq_length = tf.placeholder(tf.int32, [self.batch_size], name='hist_seq_length')

            if 'history' in config['inputs'] and config['inputs']['history']:

                hist_word_emb = utils.get_embedding(self._history,
                                            n_words=num_words,
                                            n_dim=int(config['model']['question']["embedding_dim"]),
                                            scope="anthonywasalsohere")

                hist_lstm_states, _ = rnn.variable_length_LSTM(hist_word_emb,
                                                    num_hidden=int(config['model']['question']["no_LSTM_hiddens"]),
                                                    seq_length=self._hist_seq_length,
                                                    scope="anthonywashere")
                embeddings.append(hist_lstm_states)
                print("Input: History")
