...
class DeceptiveGuesserNetwork(AbstractNetwork):

    def __init__(self, config, num_words, device='', reuse=False):
        AbstractNetwork.__init__(self, "one_who_identifies_deception", device=device)
        ...
        with tf.variable_scope(self.scope_name, reuse=reuse):
            ...
            # Targets
            self.deceptive = tf.placeholder(tf.int32, [mini_batch_size], name="deceptive")
            ...
            #self.concat_output = tf.concat([last_states, obj_embs])
            #scores = tf.matmul(obj_embs, last_states)
            with tf.variable_scope('lstm_mlp_NEW_ONE_BRO'):
                h1 = utils.fully_connected(
                    last_states,
                    n_out=256,
                    activation='relu',
                    scope='lstm_mlp_NEW_ONE_BRO1')
                h2 = utils.fully_connected(
                    h1,
                    n_out=2,
                    activation='relu',
                    scope='lstm_mlp_NEW_ONE_BRO2')

            #self.concat_output = tf.reduce_mean(h2, axis=1)

            #scores2 = tf.sigmoid(self.concat_output)
            
            #self.softmax = tf.concat([scores2, 1-scores2], axis=-1)
            ...
            self.softmax  = softmax(h2)
            self.deceptive_guess = tf.argmax(self.softmax, axis=1)

            
            self.loss = tf.reduce_mean(utils.cross_entropy(self.softmax, self.deceptive))
            self.error = tf.reduce_mean(utils.error(self.softmax, self.deceptive))
