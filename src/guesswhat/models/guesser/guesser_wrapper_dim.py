...
class DeceptiveGuesserWrapper(object):
    ...
    def is_deceptive(self, sess, dialogues, seq_length, game_data):
        game_data["dialogues"] = dialogues
        game_data["seq_length"] = seq_length
        # sample
        deceptive_guess, softmax = self.evaluator.execute(sess, output=[self.guesser.deceptive_guess, self.guesser.softmax], batch=game_data)

        #found = (selected_object == game_data["targets_index"])
        correct = (deceptive_guess == game_data["deceptive"])

        return correct, softmax, deceptive_guess
