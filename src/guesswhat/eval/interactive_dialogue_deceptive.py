...
if __name__ == '__main__':
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        if args.oracle_identifier is not None:
            oracle_config = get_config_from_xp(os.path.join(args.networks_dir, "deceptive_oracle"), args.oracle_identifier)

            oracle_network = OracleNetwork(oracle_config, num_words=tokenizer.no_words, scopename='deceptive_oracle')
            oracle_var = [v for v in tf.global_variables() if "deceptive_oracle" in v.name]
            oracle_saver = tf.train.Saver(var_list=oracle_var)

            oracle_saver.restore(sess, os.path.join(args.networks_dir, 'deceptive_oracle', args.oracle_identifier, 'params.ckpt'))
