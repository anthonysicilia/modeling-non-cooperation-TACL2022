...
answer_dict = \
    {'Yes': np.array([1, 0, 0], dtype=np.int32),
       ...
       'n': np.array([0, 1, 0], dtype=np.int32)
    }
    ...
    def apply(self, games):
        ...
        for i, game in enumerate(games):
            ...
            if 'history' in sources:
                ...
                batch['history'].append(tokenizer.apply(game.history[0]))
        ...
        if 'history' in sources:
            batch['history'], batch['hist_seq_length'] = padder(batch['history'], padding_symbol=tokenizer.word2i['<padding>'])
