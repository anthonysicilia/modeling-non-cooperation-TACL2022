...
from guesswhat.models.oracle.oracle_network_dim import OracleNetwork
...
from guesswhat.models.guesser.guesser_network_dim import DeceptiveGuesserNetwork
from guesswhat.models.looper.basic_looper_dim_w_guesser import BasicLooper
...
from guesswhat.models.oracle.dual_oracle_wrapper import DualOracleWrapper
from guesswhat.models.guesser.guesser_wrapper_dim import DeceptiveGuesserWrapper
...
if __name__ == '__main__':
    ...
    ## ADDED
    parser.add_argument("-deceptive_oracle_identifier", type=str, required=True , help='Deceptive Oracle identifier')
    ...
    parser.add_argument("-train_qgen", action="store_true", help="Train the q-gen")
    parser.add_argument("-train_guesser", action="store_true", help="Train the guesser")
    parser.add_argument("-train_on_original_task", action="store_true", help="Train qgen on original task too")
    parser.add_argument("-train_only_on_task", action="store_true", help="Train qgen on original task only")
    # trains on cooperation (deception) id, by default
    ...
    ## ADDED
    deceptive_oracle_config = get_config_from_xp(os.path.join(args.networks_dir, "deceptive_oracle"), args.deceptive_oracle_identifier)
    ...
    ## ADDED
    deceptive_oracle_network = OracleNetwork(deceptive_oracle_config, num_words=tokenizer.no_words, scopename='deceptive_oracle')
    deceptive_oracle_var = [v for v in tf.global_variables() if "deceptive_oracle" in v.name]
    deceptive_oracle_saver = tf.train.Saver(var_list=deceptive_oracle_var)

    oracle_network = OracleNetwork(oracle_config, num_words=tokenizer.no_words, scopename='oracle')
    oracle_var = [v for v in tf.global_variables() if "oracle" in v.name and "deceptive_oracle" not in v.name]
    oracle_saver = tf.train.Saver(var_list=oracle_var)

    d_guesser_network = DeceptiveGuesserNetwork(guesser_config["model"], num_words=tokenizer.no_words)
    d_guesser_var = [v for v in tf.global_variables() if "one_who_identifies_deception" in v.name]
    d_guesser_saver = tf.train.Saver(var_list=d_guesser_var)

    guesser_network = GuesserNetwork(guesser_config["model"], num_words=tokenizer.no_words)
    guesser_var = [v for v in tf.global_variables() if "guesser" in v.name and 'deceptive' not in v.name]
    ...
    #optimizer = [pg_optimize, baseline_optimize, guesser_config]
    optimizer = {}
    if args.train_qgen:
        ...
        optimizer['nota'] = [pg_optimize, baseline_optimize]
    if args.train_guesser:
        optimizer['guesser'], _ = create_optimizer(d_guesser_network, guesser_config)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        ## ADDED
        deceptive_oracle_saver.restore(sess, os.path.join(args.networks_dir, 'deceptive_oracle', args.deceptive_oracle_identifier, 'params.ckpt'))

        oracle_saver.restore(sess, os.path.join(args.networks_dir, 'oracle', args.oracle_identifier, 'params.ckpt'))
        
        # if args.restore_guesser:
        ...
        ## ADDED
        oracle_wrapper = DualOracleWrapper(oracle_network, deceptive_oracle_network, tokenizer)
        #oracle_wrapper = DualOracleWrapper(oracle_network, tokenizer)

        d_guesser_wrapper = DeceptiveGuesserWrapper(d_guesser_network)
        ...
        looper_evaluator = BasicLooper(loop_config,
                                       ...
                                       d_guesser_wrapper=d_guesser_wrapper,
                                       qgen_wrapper=qgen_wrapper,
                                       tokenizer=tokenizer,
                                       batch_size=loop_config["optimizer"]["batch_size"],
                                       train_on_original_task=args.train_on_original_task,
                                       train_only_on_task=args.train_only_on_task)
        ...
        for epoch in range(no_epoch):
            ...
            logger.info("D Accuracy (train - sampling) : {}".format(train_score[0]))
            logger.info("Accuracy (train - sampling) : {}".format(train_score[1]))
            logger.info("D Accuracy (valid - sampling) : {}".format(val_score[0]))
            logger.info("Accuracy (valid - sampling) : {}".format(val_score[1]))

            if val_score[0] > final_val_score:
                logger.info("Save checkpoint...")
                final_val_score = val_score[0]
