...
class OracleDataset(AbstractDataset):
    ...
    def split(self, game):
        ...
        for i, q, a in zip(game.question_ids, game.questions, game.answers):
            ...
            new_game.answers = [a]
            new_game.history = [' '.join(current_history)]
            current_history.append(q)
            current_history.append(a)
            games.append(new_game)
        return games

class OracleDatasetCustom(AbstractDataset):
    """
    Each game contains a single question answer pair
    """
    def __init__(self, path):
        ...
        for game in games:
            ...
            with open(game_path, 'rb') as f:
                new_games.append(pkl.load(f)[0]['game'])
        # old_games = dataset.get_data()
        # new_games = []
        # for g in old_games:
        #     new_games += self.split(g)
        super(OracleDatasetCustom, self).__init__(new_games)
